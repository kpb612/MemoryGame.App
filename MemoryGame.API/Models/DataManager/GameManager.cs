...
namespace MemoryGame.API.Models.DataManager
{
    ...
    {
        public IEnumerable<ChallengerViewModel> GetAll
        {
            get
            {
                return GetAllChallengerRank();
            }
        }
        ...
        public HTTPApiResponse UpdateCurrentBest(Rank user)
        {
            using (MemoryGameEntities db = new MemoryGameEntities())
            {
        ...
        public int GetChallengerID(string email)
        {
            using (MemoryGameEntities db = new MemoryGameEntities())
            {
                var data = db.Challengers.Where(o => o.Email.
                ToLower().Equals(email.ToLower()));
                if (data.Any())
                {
        ...
        public HTTPApiResponse AddChallenger(Challenger c)
        {
            HTTPApiResponse response = null;
            using (MemoryGameEntities db = new MemoryGameEntities())
            {
                var data = db.Challengers.Where(o => o.Email.
                ToLower().Equals(c.Email.ToLower()));
                if (data.Any())
                {
        ...
        public ChallengerViewModel GetChallengerByEmail(string email)
        {
            ...
            {
                var result = (from c in db.Challengers
                              join r in db.Ranks on c.ChallengerID
                              equals r.ChallengerID
