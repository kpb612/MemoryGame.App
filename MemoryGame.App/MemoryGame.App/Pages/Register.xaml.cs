    ...
    public partial class Register : ContentPage
    {
        ...
        async Task CheckExistingProfileAndSave(string email)
        {
            try
            {
                ...
                string.IsNullOrEmpty(player.LastName))
                {
                    ...
                    ("Error", "Email does not exist.", "OK");
                }
                else
                {
                    Settings.PlayerFirstName = player.
                    FirstName.Trim();
                    Settings.PlayerLastName = player.
                    LastName.Trim();
        ...
        async void OnbtnRegisterClicked(object sender, EventArgs args)
        {
            ...
            || string.IsNullOrEmpty(entryEmail.Text))
                ...
                DisplayAlert("Error", "Please supply the required fields.", "Got it");
            else
                await Save();
        ...
        async void OnbtnLoginClicked(object sender, EventArgs args)
        {
            ...
            else
            {
                if (Utils.IsConnectedToInternet())
                {
                    ...
                    await CheckExistingProfileAndSave
                    (entryExistingEmail.Text);
                }
                else
                {
