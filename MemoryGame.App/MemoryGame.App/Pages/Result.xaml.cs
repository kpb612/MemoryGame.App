    ...
    public partial class Result : ContentPage
    {
        ...
        async void OnButtonClicked(object sender, EventArgs args)
        {
            if (pickerLight.SelectedIndex >= 0 && pickerSpeaker.SelectedIndex >= 0 && pickerHaptic.SelectedIndex >= 0)
            {
                ...
                if (CheckAnswer(Home.CurrentGameBlinkCount,
                blinkCountAnswer))
                    if (CheckAnswer(Home.CurrentGameSoundCount, soundCountAnswer))
                        if (CheckAnswer(Home.CurrentGameHapticCount, hapticCountAnswer))
                        {
                            ...
                            var answer = await App.Current.
                            MainPage.DisplayAlert("Congrats!", $"You’ve got it all right and made it to level {Home.CurrentGameLevel}. Continue?", "Yes", "No");
                            if (answer)
                ...
                if (!_answered)
                {
                    var isSynced = PlayerManager.
                    CheckScoreAndSync(Home.CurrentGameLevel);
                    var answer = await App.Current.MainPage.
                    DisplayAlert("Game Over!", $"Your current best is at level{Home.CurrentGameLevel}. Retry?", "Yes", "No");
                    if (answer)
                        await App._navPage.PopAsync();
                    else
                        Retry();
